...
from instant_eda.model.correlations import calculate_correlation
from instant_eda.model.sample import Sample, get_sample
from instant_eda.model.summary import (
    get_duplicates,
    get_messages,
    get_missing_diagrams,
    get_scatter_matrix,
    get_time_series_matrix,
    get_series_descriptions,
    get_table_stats,
    get_missing_matrix,
)
from instant_eda.model.dtype_inference import expand_mixed, dtype_inference
...
def describe(title: str, df: pd.DataFrame, sample: Optional[dict] = None) -> dict:
    ...
    #expand cells with list, dicts or tuples
    df = expand_mixed(df)
    
    #infer dtypes
    df = dtype_inference(df)
    ...
    with tqdm(
        ...
        # Time series matrix
        pbar.set_postfix_str("Get time series matrix")
        time_series_matrix = get_time_series_matrix(df, variables)
        pbar.update()
        ...
        # missing matrix
        pbar.set_postfix_str("Get missing interaction diagrams")
        missing_interactions = get_missing_matrix(df, variables)
        pbar.update()        
