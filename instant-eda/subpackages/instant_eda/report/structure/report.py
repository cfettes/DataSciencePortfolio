...
from instant_eda.model.base import (
    AbsolutePath,
    Boolean,
    Categorical,
    Complex,
...
from instant_eda.model.messages import MessageType
from instant_eda.report.presentation.core import (
    HTML,
    Collapse,
    Container,
    Duplicate,
...
from instant_eda.report.structure.correlations import get_correlation_items
from instant_eda.report.structure.overview import get_dataset_items
from instant_eda.report.structure.variables import (
    render_boolean,
    render_categorical,
    render_complex,
    render_date,
...
from instant_eda.report.structure.variables.render_file import render_file
...
def get_time_series_matrix(time_series_matrix: dict) -> list:
    """Returns the time series components for the report

    Args:
        time_series_matrix: a nested dict containing the time series plots

    Returns:
        A list of components for the time series section of the report
    """
    image_format = config["plot"]["image_format"].get(str)
    ...
    for x_col, y_cols in time_series_matrix.items():
        items = []
        for y_col, splot in y_cols.items():
            items.append(
                Image(
                    ...
                    image_format=image_format,
                    alt=f"{x_col} x {y_col}",
                    anchor_id=f"time_{clean_name(x_col)}_{clean_name(y_col)}",
                    name=y_col,
                )
            )
        ...
        titems.append(
            Container(
                ...
                sequence_type="tabs" if len(items) <= 10 else "select",
                name=x_col,
                nested=len(time_series_matrix) > 10,
                anchor_id=f"time_{clean_name(x_col)}",
            )
        )
    return titems

def get_missing_matrix(missing_matrix: dict) -> list:
    """Returns the time series components for the report
    ...
    for x_col, y_cols in missing_matrix.items():
        items = []
        for y_col, splot in y_cols.items():
            items.append(
                Image(
                    ...
                    image_format=image_format,
                    alt=f"{x_col} x {y_col}",
                    anchor_id=f"missing_interactions_{clean_name(x_col)}_{clean_name(y_col)}",
                    name=y_col,
                )
            )
        ...
        titems.append(
            Container(
                ...
                sequence_type="tabs" if len(items) <= 10 else "select",
                name=x_col,
                nested=len(missing_matrix) > 10,
                anchor_id=f"missing_interactions_{clean_name(x_col)}",
            )
        )
    return titems
...
def get_report_structure(summary: dict) -> Renderable:
    ...
    with tqdm(
        ...
        time_items = get_time_series_matrix(summary["time"])
        if len(time_items) > 0:
            section_items.append(
                Container(
                    time_items,
                    sequence_type="tabs" if len(time_items) <= 10 else "select",
                    name="Time series",
                    anchor_id="time",
                ),
            )
        ...
        missing_interactions = get_missing_matrix(summary["missing_interactions"])
        if len(missing_interactions) > 0:
            section_items.append(
                Container(
                    missing_interactions,
                    sequence_type="tabs", #if len(missing_interactions) <= 10 else "select",
                    name="Missing Value Interactions",
                    anchor_id="missing_interactions",
                ),
            )
    ...
    footer = HTML(
        content='Report generated for JBC.'
    )
