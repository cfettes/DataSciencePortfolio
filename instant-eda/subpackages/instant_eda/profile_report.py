...
from instant_eda.model.describe import describe as describe_df
from instant_eda.model.messages import MessageType
from instant_eda.report import get_report_structure
from instant_eda.report.presentation.flavours.html.templates import (
    create_html_assets,
)
from instant_eda.serialize_report import SerializeReport
from instant_eda.utils.dataframe import hash_dataframe, rename_index
from instant_eda.utils.paths import get_config


class InstantEDA(SerializeReport, object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Generate a InstantEDA report based on a pandas DataFrame
        ...
        if content=='full':
            ...
            config.set_file(get_config("config_missing.yaml"))
        elif content=='time':
            config.set_file(get_config("config_time.yaml"))
        #elif minimal:
        elif content == 'minimal':
            config.set_file(get_config("config_minimal.yaml"))
        elif content == 'interaction':
            config.set_file(get_config("config_interaction.yaml"))
        else:
            #config.set_file(config_file)
            config.set_file(get_config("config_default.yaml"))
    ...
    def set_variable(self, key, value):
        ...
        Examples:
            >>> InstantEDA(df).set_variables("title", "NewTitle")
            >>> InstantEDA(df).set_variables("html", {"minify_html": False})
            >>> InstantEDA(df).set_variables("html.minify_html", False)
    ...
    def set_variables(self, **vars):
        ...
        Examples:
            >>> InstantEDA(df).set_variables(title="NewTitle", html={"minify_html": False})
        """
    ...
    def _render_html(self):
        from instant_eda.report.presentation.flavours import HTMLReport
        ...
        with tqdm(total=1, desc="Render HTML", disable=disable_progress_bar) as pbar:
            html = HTMLReport(report).render(
                ...
                #version=self.description_set["package"]["instant_eda_version"],
            )
    ...
    def _render_widgets(self):
        from instant_eda.report.presentation.flavours import WidgetReport
    ...
    def to_notebook_iframe(self):
        ...
        from instant_eda.report.presentation.flavours.widget.notebook import (
            get_notebook_iframe,
        )
    ...
    def to_widgets(self):
        ...
        try:
            ...
            warnings.warn(
                "Ipywidgets is not fully supported on Google Colab (https://github.com/googlecolab/colabtools/issues/60)."
